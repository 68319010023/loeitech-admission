...
export const createExpenseDetail = async (req: Request, res: Response) => {
  try {
    ...
    const query = `
      INSERT INTO expense_detail (exp_name, exp_detail, exp_img, cur_id, exp_cost)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `
    const result = await pool.query(query, [exp_name, exp_detail, exp_img, cur_id, exp_cost])
...
export const updateExpenseDetail = async (req: Request, res: Response) => {
  try {
    ...
    const query = `
      ...
      SET exp_name = $1, exp_detail = $2, exp_img = $3, cur_id = $4, exp_cost = $5
      WHERE exp_id = $6
      RETURNING *
    `
    const result = await pool.query(query, [exp_name, exp_detail, exp_img, cur_id, exp_cost, id])
